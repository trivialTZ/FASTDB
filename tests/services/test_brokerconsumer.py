...
from services.brokerconsumer import BrokerConsumer, BrokerConsumerLauncher, LigoVirgoKagraConsumer
...
def test_BrokerConsumerLauncher(
    barf, alerts_30days_sent_and_classified, mongoclient, mongoclient_rw
):
    dbname = os.getenv("MONGODB_DBNAME")
    assert dbname is not None
    collection = f"fastdb_{barf}"
    ...
    try:
        ...
        def launch_launcher():
            bcl = BrokerConsumerLauncher(
                "/code/tests/services/brokerconsumer.yaml",
                barf=barf,
                logtag="BrokerConsumerLauncher",
                verbose=True,
            )
            bcl()
...
def test_LigoVirgoKagraConsumer(barf, mongoclient, mongoclient_rw):
    ...
    with tempfile.NamedTemporaryFile("w", delete=False) as passwdfile:
        passwdfile.write(password + "\n")
        passwd_path = passwdfile.name

    try:
        # First, make sure it times out properly if it never sees a topic
        t0 = time.perf_counter()
        bc = LigoVirgoKagraConsumer(
            grouptag=barf,
            usernamefile=username_path,
            passwdfile=passwd_path,
            loggername="LVKTEST",
            lvk_topic="this_topic_does_not_exist",
            mongodb_collection=collection,
            nomsg_sleeptime=1,
        )
        bc.poll(restart_time=datetime.timedelta(seconds=3), max_restarts=2, notopic_sleeptime=2)
